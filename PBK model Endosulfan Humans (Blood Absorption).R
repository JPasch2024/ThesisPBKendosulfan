...
InitModel <- function() {
  model2 <- RxODE({
    ...
    # For Endosulfan_Beta
    d/dt(Abl_B) <- (Ka_B*Ain1_B + Ka_B*Ain2_B + Ka_B*Ain3_B + Ka_B*Ain4_B + Ka_B*Ain5_B + Ka_B*Ain6_B + Ka_B*Ain7_B - Kb_B * Ali_B) + ((Qli * (Cli_v_B - Cbl_B)) + (Qki * (Cki_v_B - Cbl_B)) + (Qbr * (Cbr_v_B - Cbl_B)) + (Qf * (Cf_v_B - Cbl_B)) + (Qt * (Ct_v_B - Cbl_B)) + (Qwp * (Cwp_v_B - Cbl_B)) + (Qpp * (Cpp_v_B - Cbl_B)));
    ...
    d/dt(Ali_B) <- Qli * (Cbl_B - Cli_v_B) - AMR_B;
...
# outputBlood <- sim_data_human$Cbl_A + sim_data_human$Cbl_B
# outputBrain <- sim_data_human$Cbr_A + sim_data_human$Cbr_B
# outputTestes <- sim_data_human$Ct_A + sim_data_human$Ct_B
# outputFat <- sim_data_human$Cf_A + sim_data_human$Cf_B
# 
# output_data <- data.frame(Time = sim_data_human$time, Outputf = outputFat)#Outputbl = outputBlood, Outputbr = outputBrain, Outputt = outputTestes)
# 
# # Write the data to an Excel file
# write.xlsx(output_data, file = "outputHumanFat.csv", rowNames = FALSE)
# 
