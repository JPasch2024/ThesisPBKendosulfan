...
Vmax_B <- Vmax_A / 0.336339044 #umol/h #Same relative metabolism assumed as with human in vitro data
Km_B <- Km_A / 1.1523 #uM


Vmax_S  <- Vmax_A * 0.5 #TEMP  DATA IS LACKING. 
Km_S <- 1.0 #TEMP
...
InitModel <- function() {
  model2 <- RxODE({
    ...
    d/dt(Ali_B) <- (Ka_B*Ain1_B + Ka_B*Ain2_B + Ka_B*Ain3_B + Ka_B*Ain4_B + Ka_B*Ain5_B + Ka_B*Ain6_B + Ka_B*Ain7_B) +
    Qli * (Cbl_B - Cli_v_B) - AMR_B - Kb_B * Ali_B;
...
compartment_name <- "Cbl_B"
compartment_name2 <- "Cbl_B" #Compartments can be added to calculate the sums of the different endosulfan compounds. 
...
# Assuming df is your data frame and ColumnName is the column you want
column_data <- sim_data_Rats$TAMR_A

# Export data to CSV file
write.csv(column_data, file = "output.csv", row.names = FALSE)

# Copy data to clipboard
write.table(column_data, file = "clipboard", sep = "\t", row.names = FALSE)

# Display a message indicating the process is complete
cat("Data exported to output.csv and copied to clipboard.\n")
# 
# Oral_Dose_A_Absorbed
...
#   lsa <- sensFun(LSArun, 
#                  parms = pars,
#                  sensvar = c("Cbl_S"), #Output variable I want testes; What do I want testes? Cmax of blood conc standard?
#                  senspar = names(pars),
#                  varscale = NULL, 
#                  parscale = NULL, 
...
#   
#   Tmax <- solve.pbk[which.max(solve.pbk$Cbl_S), "time"] #ModelOutput? Should it be as.data.frame(solve.pbk) for my model?
#   
